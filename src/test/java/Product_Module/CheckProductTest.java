...
public class CheckProductTest {
	...
	/*	FileInputStream fi=new FileInputStream(".\\src\\test\\resources\\testdata.xlsx");
		...
		String procode=s.getRow(0).getCell(1).getStringCellValue();
		procode=procode+	jLib.getRandomNo();
	...
	/*	String procode=eLib.readDataFromExcel("Product",0 , 1);
		...
		int count = eLib.getLastRowNo("Product1");*/
	//storing in hash map(excel)
	/*	int count = eLib.getLastRowNo("Product1");
		HashMap<String,String> map=new HashMap<String,String>();
		for(int i=0;i<=count;i++) {
			//String key=s.getRow(i).getCell(0).getStringCellValue();
			String key=	eLib.getMultipleDataFromExcel("Product1", i, 1);
			eLib.getMultipleDataFromExcel("Product1", i, count)
		...
		eLib.getMultipleDataFromExcel("Product1", 1, 2);
